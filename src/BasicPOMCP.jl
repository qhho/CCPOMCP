...
"""
const CPOMDPType = ConstrainedPOMDPWrapper #set type of ConstrainedPOMDP
...
struct POMCPTree{A,O}
    ...
    c::Vector{Vector{Int64}}             # Cost estimate for an action node
    c_bar::Vector{Vector{Int64}}         # Average immediate cost
end

function POMCPTree(c_pomdp::CPOMDPType, b, sz::Int=1000)
    pomdp = c_pomdp.m
...
function insert_obs_node!(t::POMCPTree, c_pomdp::CPOMDPType, ha::Int, sp, o)
    pomdp = c_pomdp.m
...
mutable struct POMCPPlanner{P, SE, RNG} <: Policy
    solver::POMCPSolver
    c_problem::CPOMDPType #????
    problem::P #::typeof(P.m) #does this work
    solved_estimator::SE
    rng::RNG
...
function POMCPPlanner(solver::POMCPSolver, c_pomdp::CPOMDPType)
    se = convert_estimator(solver.estimate_value, solver, c_pomdp.m) #Not sure what this is
    return POMCPPlanner(solver, c_pomdp, c_pomdp.m, se, solver.rng, Int[], nothing)
end
