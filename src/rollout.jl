...
function estimate_value(estimator::Union{SolvedPORollout,SolvedFORollout}, pomdp::ConstrainedPOMDPs.ConstrainedPOMDPWrapper, start_state, h::BeliefNode, steps::Int)
    # println("called")
    # println(estimator)
    rollout(estimator, pomdp, start_state, h, steps)
end

@POMDP_require estimate_value(estimator::Union{SolvedPORollout,SolvedFORollout}, pomdp::ConstrainedPOMDPs.ConstrainedPOMDPWrapper, start_state, h::BeliefNode, steps::Int) begin
    @subreq rollout(estimator, pomdp, start_state, h, steps)
end

# function estimate_value(estimator::SolvedFOValue, pomdp::ConstrainedPOMDPs.ConstrainedPOMDPWrapper, start_state, h::BeliefNode, steps::Int)
#     POMDPs.value(estimator.policy, start_state) ####THIS
# end


function convert_estimator(ev::RolloutEstimator, solver, pomdp) #Solves and wraps
...
"""
function rollout(est::SolvedPORollout, c_pomdp::ConstrainedPOMDPs.ConstrainedPOMDPWrapper, start_state, h::BeliefNode, steps::Int)
    # println("also called")
